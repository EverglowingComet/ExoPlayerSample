...
final class CustomExtractorMediaPeriod implements MediaPeriod, ExtractorOutput,
        Loader.Callback<CustomExtractorMediaPeriod.ExtractingLoadable>, Loader.ReleaseCallback,
    ...
    private final CustomExtractorMediaPeriod.Listener listener;
    private final Allocator allocator;
    @Nullable private final String customCacheKey;
    private final long continueLoadingCheckIntervalBytes;
    private final Loader loader;
    private final CustomExtractorMediaPeriod.ExtractorHolder extractorHolder;
     ...
     */
    public CustomExtractorMediaPeriod(
            Uri uri,
            DataSource dataSource,
            Extractor[] extractors,
            int minLoadableRetryCount,
            EventDispatcher eventDispatcher,
            CustomExtractorMediaPeriod.Listener listener,
            Allocator allocator,
        ...
        loader = new Loader("Loader:CustomExtractorMediaPeriod");
        extractorHolder = new CustomExtractorMediaPeriod.ExtractorHolder(extractors, this);
        ...
        onContinueLoadingRequestedRunnable = new Runnable() {
            ...
            public void run() {
                if (!released) {
                    callback.onContinueLoadingRequested(CustomExtractorMediaPeriod.this);
                }
            }
        };
        handler = new Handler();
    ...
    public long selectTracks(TrackSelection[] selections, boolean[] mayRetainStreamFlags,
        ...
        for (int i = 0; i < selections.length; i++) {
            if (streams[i] != null && (selections[i] == null || !mayRetainStreamFlags[i])) {
                int track = ((CustomExtractorMediaPeriod.SampleStreamImpl) streams[i]).track;
        ...
        for (int i = 0; i < selections.length; i++) {
            if (streams[i] == null && selections[i] != null) {
                ...
                streams[i] = new CustomExtractorMediaPeriod.SampleStreamImpl(track);
    ...
    @Override
    public void onLoadCompleted(CustomExtractorMediaPeriod.ExtractingLoadable loadable, long elapsedRealtimeMs,
                                long loadDurationMs) {
    ...
    @Override
    public void onLoadCanceled(CustomExtractorMediaPeriod.ExtractingLoadable loadable, long elapsedRealtimeMs,
    ...
    @Override
    public int onLoadError(CustomExtractorMediaPeriod.ExtractingLoadable loadable, long elapsedRealtimeMs,
    ...
    private void copyLengthFromLoader(CustomExtractorMediaPeriod.ExtractingLoadable loadable) {
    ...
    private void startLoading() {
        CustomExtractorMediaPeriod.ExtractingLoadable loadable = new CustomExtractorMediaPeriod.ExtractingLoadable(uri, dataSource, extractorHolder,
                loadCondition);
        if (prepared) {
     ...
     *     retry.
     */
    private boolean configureRetry(CustomExtractorMediaPeriod.ExtractingLoadable loadable, int currentExtractedSampleCount) {
        if (length != C.LENGTH_UNSET
    ...
    private final class SampleStreamImpl implements SampleStream {
        ...
        public boolean isReady() {
            return CustomExtractorMediaPeriod.this.isReady(track);
        }

        @Override
        public void maybeThrowError() throws IOException {
            CustomExtractorMediaPeriod.this.maybeThrowError();
        }

        @Override
        public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer,
                            boolean formatRequired) {
            return CustomExtractorMediaPeriod.this.readData(track, formatHolder, buffer, formatRequired);
        }

        @Override
        public int skipData(long positionUs) {
            return CustomExtractorMediaPeriod.this.skipData(track, positionUs);
        }
    ...
    /* package */ final class ExtractingLoadable implements Loadable {
        ...
        public ExtractingLoadable(Uri uri, DataSource dataSource, CustomExtractorMediaPeriod.ExtractorHolder extractorHolder,
